...
export const POST = async (req: NextRequest) => {
  ...
  const { amount, credits, userEmail, itemName } = await req.json(); // Use .json() to parse request body
  const orderId = randomUUID(); // Generate unique order ID
  ...
  // Convert to query string while maintaining key order
...
// Helper function to generate the PayFast signature
function generateSignature(data: any, passPhrase: string | null = null) {
  const keys = [
    "merchant_id",
    "merchant_key",
    "return_url",
